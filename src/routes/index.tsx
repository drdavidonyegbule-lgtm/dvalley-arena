...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "keywords", content: "D Valley's Arena, lounge Bori Rivers State, nightclub Bori, restaurant Bori, fitness gym Bori, lifestyle destination Rivers State, premium lounge Port Harcourt" },
        ...
        <div className="flex marquee gap-4 w-max">
          {[...gallery, ...gallery].map((src, i) => (
            <div key={i} className="w-[300px] h-[380px] shrink-0 overflow-hidden rounded-sm border border-white/10">
              <img src={src} alt="D' Valley's Arena scene" className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
      </section>
